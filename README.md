...
The implement_detection function loads the car detection YOLO model.

It reads the input video frame by frame.

On each frame, it runs the model to detect objects.

It filters these detections to isolate only the 'Car' class.

A green bounding box and a confidence label are drawn onto the frame for each detected car.
...
The process_video function analyzes the new video. Note: Instead of re-running object detection, it uses OpenCV's color detection (cv2.inRange) to find the green bounding boxes drawn in Step 1.

It tracks these boxes using their centroids to identify movement.

It selects the largest car that is currently moving.

The is_car_near function checks if the selected car is "near" the camera based on its bounding box area, height, and distance from the bottom of the frame.

If a car is deemed "near," the mask_car function is called to create a new image containing only the car, with the rest of the frame blacked out.
...
The script iterates through each car image from Step 2.

It uses the license plate YOLO model to find a plate (class ID 0) on the car.

Once a plate is found, it's cropped from the car image.
...
OCR: The preprocessed plate image is passed to the TrOCR model (trocr_predict) to extract the text.

Validation: The recognized text is cleaned and validated by the correct_plate_format function, which checks it against standard Indian plate regex (pattern_standard), Bharat (BH) series regex (pattern_bh), and a set of valid_state_codes.

Output: For each valid license plate, a matplotlib plot is displayed showing the original car image, the cropped plate, and the final validated OCR text. Invalid or unread plates are skipped.
...
Ensure you have all dependencies installed (run Cell 1).

Make sure the necessary files are in the correct paths (e.g., /content/):
